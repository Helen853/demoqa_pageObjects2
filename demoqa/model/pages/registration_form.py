...
from demoqa.utils import pict_path
from demoqa.model.controls import dropdown, date_picker
...
class RegistrationForm():
    def register(self, user: User):
        ...
        self.set_number(user.phone_number)
        self.set_birthday(user.date_of_birth)
        self.set_subject(user.subject)
        self.set_hobbies(user.hobby)
    ...
    def set_birthday(self, date):
        ...
        date_picker.set_month(date[1])
        date_picker.set_year(date[2])
        date_picker.set_day(date[1], date[0])
        return self
    ...
    def upload_picture(self, path):
        s('#uploadPicture').send_keys(pict_path.take_path(path))
        return self
